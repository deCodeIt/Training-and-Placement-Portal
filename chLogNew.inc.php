...
if(isCompany() && isset($_POST["submit"])) {
	...
	$newName= rand(1000,9999) . md5($_SESSION['company_id']) . rand(1000,9999) .".". $imageFileType;
	...
	// if everything is ok, try to upload file
		...
		$img_old=getCompanyField('comp_logo');
		
			# code...
			if($img_old!="default_company_logo.png" && !unlink($target_dir.$img_old))
			{
				$uploadOk=0;
				$statusMSG="Sorry, Unable to replace your Old profile pic.";
			}
			else
			{
				if (move_uploaded_file($_FILES["InputFile"]["tmp_name"], $target_file)) {
			        if(setCompanyField('comp_logo',$newName))
			        {
		 ...
		 <form class="form-horizontal" role="form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">
		  ...
		  <div class="form-group">
		    <div class=" col-xs-offset-4 col-xs-4 col-md-offset-1 col-md-3">
		      <input type="submit" class="btn btn-success btn-block" name="submit" value="Change">
		    </div>
		  </div>
		</form>
		</div>
