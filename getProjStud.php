...
function getTabled($myVal)
{
	...
	$row_status='';
	$states=0;
	switch ($states=getLinkStudent($myVal["id"])) {
		case '0':
			$row_status='text-warning bg-warning';
			break;
		case '1':
			$row_status='text-success bg-success';
			break;
		case '2':
			$row_status='text-danger bg-danger';
			break;
		default:
			$row_status='';
			break;
	...
	$states=$row_status;
	echo '<tr class="'.$row_status.'" onclick="changeModalText(\''.$myVal['description'].'\',\''.$myVal["stipend"].'\',\''.getCompanyDetails("company_name",$myVal["company_email_id"]).'\',\''.$myVal['id'].'\',\''.$states.'\',\''.$myVal["company_email_id"].'\');" data-target="#myModal" data-toggle="modal"><td>'.$ct.'</td>
