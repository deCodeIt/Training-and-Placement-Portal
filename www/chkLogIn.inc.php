...
function _getLinkStudentProject($id,$connect)
{
	global $table_name_studentProject;
	$myStudList=array();
	$myquery=sprintf("SELECT entry_number FROM %s WHERE id='%s'",$table_name_studentProject,$id);
	if($resQuery=mysqli_query($connect,$myquery))
	{
		if($myValue = mysqli_fetch_row($resQuery))
		{
...
{
	$st='[\'Student Name\',\'CV-Resume\'';
	#$st='<table class="table" id="st_names"><tr><th>Student Name</th><th>CV-Resume</th></tr>';
	$len = count($entryList);
	for ($i=0; $i < $len; $i++) {
		$temp_st=',\''._getStudentFieldSelf('name',$entryList[$i][0],$connect).'\',\'/cv_students/'._getStudentFieldSelf('cv_link',$entryList[$i][0],$connect).'\'';
		$st=$st.$temp_st;
	}
	#$st=$st.'</table>';
	$st=$st.']';
